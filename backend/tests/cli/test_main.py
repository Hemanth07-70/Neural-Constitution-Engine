...
from backend.cli.main import main, handle_validate, handle_evaluate, handle_explain
...
class TestCLI(unittest.TestCase):
    def setUp(self):
        ...
        self.req_path.write_text(json.dumps({
            "api_version": "nce/v1",
            "action": {"type": "test"}
        }))
    ...
    def test_validate_success(self, mock_stdout):
    ...
    def test_validate_fail_missing_file(self, mock_stderr):
    ...
    @patch("sys.stdout", new_callable=StringIO)
    def test_evaluate_success(self, mock_stdout):
    ...
    @patch("sys.stderr", new_callable=StringIO)
    def test_evaluate_fail_request(self, mock_stderr):
    ...
    @patch("sys.stdout", new_callable=StringIO)
    def test_explain_success(self, mock_stdout):
